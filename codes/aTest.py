...
class mainApp(MDApp):
    def build(self):
        ...
        self.test = Label()
        self.inp = TextInput()
        #self.butt = Button( text = "click ", pos_hint= {'center_x':0.5, 'center_y': 0.5},size_hint = (None, None))
        layout.add_widget(self.image)
        layout.add_widget(self.label)
        layout.add_widget(self.inp)
        layout.add_widget(self.test)
        
        #self.butt.bind(on_press = self.enter)
        #layout.add_widget(self.butt)
        #aaa = URL + ":81/stream"
        self.capture = cv2.VideoCapture("http://192.168.0.128:81/stream")
        Clock.schedule_interval(self.loadVid , 1.0/12.0)
        return layout
    ...
    def loadVid(self,*args):
        ...
        if face is not None: # Transform for model input
            ...
            with torch.no_grad():
                ...
                self.label.text =predicted_label
